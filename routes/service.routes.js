...
router.post('/vote', auth, async (req, res) => {
  try {
    ...
    const langIndex = ruCountry.attractions.findIndex(
      (attraction) => String(attraction._id) === attractionId
    );
    ...
    // if already voted
    const ratingIndex = enAttraction.rating.findIndex(
      (rateObject) => String(rateObject.userId) === String(userId)
    );
